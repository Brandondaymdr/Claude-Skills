...
def circle_get_all(endpoint, params=None):
    """GET with automatic pagination — fetches all pages."""
    all_records = []
    page = 1
    ...
    while True:
        ...
        data = circle_get(endpoint, base_params)
        records = data.get("records", data) if isinstance(data, dict) else data
        if not records:
            break
        all_records.extend(records)
        if isinstance(data, dict) and not data.get("has_next_page", False):
            break
        if len(records) < 50:
            break
        page += 1
...
def find_and_tag_member(email, tag_name):
    ...
    records = members.get("records", members) if isinstance(members, dict) else members
    if not records:
        raise ValueError(f"No member found with email: {email}")
    member_id = records[0]["id"]

    # Find or create tag
    tags_data = circle_get("member_tags")
    tags = tags_data.get("records", tags_data) if isinstance(tags_data, dict) else tags_data
...
```python
def create_announcement(space_id, title, body_html):
...
def upgrade_member(member_id, old_group_id, new_group_id):
    ...
    circle_delete(
        f"access_groups/{old_group_id}/community_members",
        {"community_member_id": member_id}
    )
    # Add to new group
    circle_post(
        f"access_groups/{new_group_id}/community_members",
        {"community_member_id": member_id}
    )
...
def add_all_members_to_space(space_id):
    ...
    members = circle_get_all("community_members")
...
def community_report():
    ...
    members = circle_get_all("community_members")
    spaces_data = circle_get("spaces")
    spaces = spaces_data.get("records", spaces_data) if isinstance(spaces_data, dict) else spaces_data
    access_groups_data = circle_get("access_groups")
    access_groups = access_groups_data.get("records", access_groups_data) if isinstance(access_groups_data, dict) else access_groups_data
...
def export_members_csv(output_path):
    ...
    member_tags = {}
    for tag in tags:
        tagged_data = circle_get("tagged_members", {"member_tag_id": tag["id"]})
        tagged = tagged_data.get("records", tagged_data) if isinstance(tagged_data, dict) else tagged_data
        for tm in tagged:
